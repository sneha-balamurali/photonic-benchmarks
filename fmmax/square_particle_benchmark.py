"""FMMax implementation of a 2D periodic square metal particle

To begin, we will use the same layer stack and materials as in 
the 1D metal grating, but we are replacing the infinitely extended
stripe with a finite square centred in each 180nm x 180nm unit cell.


The physical structure consists of:

- Square lattice period: 180nm in both x and y
- Metal square: 60nm x 60nm
- Patterned-layer thickness: 80nm
- Uniform planarization layer: 20nm
- Ambient: epsilon = 1
- Background dielectric: epsilon = 2.25
- Square and substrate: epsilon = -7.632 + 0.731i
- Wavelength = 500nm
- Normal incidence

Semi-infinite ambient
-------------------------
20 nm planarization layer
-------------------------
80 nm patterned layer:
60 × 60 nm metal square
in dielectric background
---------------------------
Semi-infinite metal substrate
"""
...
def simulate_grating(
    ...
    """Computes the TE- and TM-polarized reflection from a 2D periodic square particle layer
    for a specified Fourier-basis target, truncation, and FMM formulation. 

    It:
    1. Rasterises the square particle on a 2D unit-cell grid
    ...
    Args:
        ...
        particle_width_nm: The width of the the square metal particle in nanometers.
    ...
    density = (
                (jnp.abs(x_nm) <= particle_width_nm / 2) &
                (jnp.abs(y_nm) <= particle_width_nm / 2)
    ).astype(float)
...
def convergence_study(
    ...
    output_path = results_directory / "square_particle_comparison_1.csv"
...
def wavelength_sweep(
    ...
    output_path = results_directory / "square_particle_spectrum.csv"
...
def plot_results(results):
    """Plots convergence and runtime for the FFT formulation for square particle."""
    ...
    plt.title("Convergence of FFT Formulation for Square Particle Simulation")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/fft_convergence_plot_square_particle.png", dpi=200)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/fft_runtime_plot_square_particle.png", dpi=200)
    plt.close()
