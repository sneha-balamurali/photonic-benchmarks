"""Originally based on the FMMax metal_grating.py example. 
...
def contains_nonfinite(array: jnp.ndarray) -> bool:
    """Return True if an array contains NaN or infinity."""
    return bool(jnp.any(~jnp.isfinite(array)))


def report_array(name: str, array: jnp.ndarray) -> None:
    """Print whether an array contains NaN or infinity."""
    print(
        f"{name}: shape={array.shape}, "
        f"nonfinite={contains_nonfinite(array)}"
    )
...
def simulate_grating(
    ...
    for layer_name, p in zip(layer_names, permittivities):
        ...
        print(f"\nLayer: {layer_name}")
        report_array("  eigenvalues", result.eigenvalues)
        report_array("  eigenvectors", result.eigenvectors)
        report_array(
            "  z_permittivity_matrix",
            result.z_permittivity_matrix,
        )
        report_array(
            "  inverse_z_permittivity_matrix",
            result.inverse_z_permittivity_matrix,
        )

        if result.tangent_vector_field is not None:
            tx, ty = result.tangent_vector_field
            report_array("  tangent tx", tx)
            report_array("  tangent ty", ty)

        layer_solve_results.append(result)
